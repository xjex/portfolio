...
const Footers = () => {
	...
	const checkIfEmailExists = async () => {
		let { data: subscriptions, error } = await supabase.from('subscriptions').select('email').eq('email', email);
		if (error) {
			setModaltitle('Error');
	...
	const sendEmailData = async () => {
		const { data, error } = await supabase.from('subscriptions').insert([{ email: email }]);
		if (error) {
			setModaltitle('Error');
			setModalcontent('Please try again');
			setShowModal(true);
			...
			setModalcontent('I will keep you updated.');
			setShowModal(true);
		}
	};
						...
						<div className='relative w-auto my-6 mx-auto max-w-3xl'>
							...
							<div className='border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none'>
								...
								<div className='relative p-6 flex-auto'>
									<p className='my-4 text-slate-500 text-lg leading-relaxed'>{modalcontent}</p>
								</div>
								{/*footer*/}
