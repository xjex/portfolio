...
const BlogContent = () => {
	...
	const formatDate = date => {
		...
		if (seconds < 60) {
			...
			return years + yearsText + (remainingMonths > 0 ? ' and ' + remainingMonths + monthsText : '') + ' ago';
		}
	};
	...
	useEffect(() => {
		const getBlog = async () => {
			try {
				const { data, error } = await supabase.from('Post').select('*').eq('id', getID);
	...
	return (
		<div className=' bg-gray-500 h-full '>
			<div className='flex justify-center items-center '>
				<h1 className='text-4xl font-bold text-white mt-10 mb-5'>{blog !== null && blog.title}</h1>
			</div>
