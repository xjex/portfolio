...
const Nav = () => {
	useEffect(() => {
		...
		if (close.length) {
			for (let i = 0; i < close.length; i++) {
				close[i].addEventListener('click', handleCloseClick);
			}
		}
		...
		return () => {
			...
			if (close.length) {
				for (let i = 0; i < close.length; i++) {
					close[i].removeEventListener('click', handleCloseClick);
				}
			}
	...
	return (
		<div className='bg-gray-700 sticky'>
			<nav className='relative px-4 py-4 flex justify-between items-center bg-white rounded-b-lg '>
				...
				<ul className='hidden absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 lg:flex lg:mx-auto lg:flex lg:items-center lg:w-auto lg:space-x-6'>
					...
					<li>
						<a className='text-sm text-gray-400 hover:text-gray-500' href='/blogs'>
							Blogs
						</a>
					</li>
					<li className='text-gray-300'>
